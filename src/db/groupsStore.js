...
const distinctWatchedFixturesStmt = db.prepare(`SELECT DISTINCT fixtureId FROM groups WHERE fixtureId IS NOT NULL`);
...
export const GroupsStore = {
  ...
  /** Every distinct fixture at least one group is currently watching — used by the ambient-commentary scheduler. */
  watchedFixtures() {
    return distinctWatchedFixturesStmt.all().map((r) => r.fixtureId);
  },
};
