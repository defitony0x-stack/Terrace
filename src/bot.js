...
import { recordMessage, recordMoment, getRecentMessages, getRecentMoments } from "./chatMemory.js";
import { generateReply, generateMomentCommentary } from "./ai.js";
...
export function createBot() {
  ...
  bot.on("message:text", async (ctx) => {
    ...
    if (!wasMentioned && !wasReplied) return;
    ...
    const reply = await generateReply({
      ...
      recentMoments: getRecentMoments(chatId),
      matchLabel,
    });
...
export async function broadcastMoment(bot, moment) {
  ...
  for (const chatId of chatIds) {
    // Record the plain fallback line into history regardless of which
    // version actually gets sent — keeps future commentary/replies
    // grounded in a consistent, factual record of what happened.
    recordMoment(chatId, fallback);
    ...
    bot.api.sendMessage(chatId, commentary || fallback).catch((err) =>
